...
export const Layout: React.FC = () => {
  ...
  const handleCreateProcurement = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Backend evaluation error fallback:", err.message);
      setIsNewProcurementOpen(false);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <Modal
        ...
        description="Initiate an intelligent procurement analysis with ProcuraIQ risk modeling."
      >
        <form onSubmit={handleCreateProcurement} className="space-y-3.5 mt-2">
          <div>
            <label className="block text-[12px] font-semibold text-text mb-1">Procurement Category</label>
            <select 
              value={category} 
          ...
          <div>
            <label className="block text-[12px] font-semibold text-text mb-1">Item / Service Description</label>
            <input 
              required
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-[12px] font-semibold text-text mb-1">Quantity</label>
              <input 
                type="number" 
                value={quantity}
            ...
            <div>
              <label className="block text-[12px] font-semibold text-text mb-1">Unit Price (₹)</label>
              <input 
                type="number" 
                value={unitPrice}
            ...
            <div>
              <label className="block text-[12px] font-semibold text-text mb-1">Lead Time (Days)</label>
              <input 
                type="number" 
                value={leadTime}
          ...
          <div className="pt-3 flex justify-end gap-2 border-t border-border">
            ...
            <Button type="submit" variant="primary" disabled={evaluating} className="flex items-center gap-1.5">
              {evaluating && <Loader2 size={14} className="animate-spin" />}
              {evaluating ? 'Evaluating Model...' : 'Run ProcuraIQ Evaluation'}
            </Button>
          </div>
        </form>
      </Modal>
